import { getMovieDetails, getSuggestedMovies, getMoviesList } from "../axios";
...
export default {
  ...
  getters: {
    ...
    pages: ({ res }) => (res ? Math.round(res.movie_count / res.limit) : 1),
  ...
  actions: {
    loadMoviesList({ commit }, options) {
      return getMoviesList(options).then(res => {
